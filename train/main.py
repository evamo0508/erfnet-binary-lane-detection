...
from dataset import VOC12, cityscapes,avlane
...
NUM_CHANNELS = 3
NUM_CLASSES = 1 #pascal=22, cityscapes=20, autovalet=1 (binary)
...
class MyCoTransform(object):
    ...
    def __call__(self, input, target):
        ...
        if(self.augment):
            ...
            target = ImageOps.expand(target, border=(transX,transY,0,0), fill=0) #pad label filling with 0
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        for step, (images, labels) in enumerate(loader):
            ...
            if (doIouTrain):
                ...
                iouEvalTrain.addBatch(preds.squeeze(), targets[:, 0]) # no_grad handles it already
        ...
        with torch.no_grad():
            for step, (images, labels) in enumerate(loader_val):
                ...
                if (doIouVal):
                    ...
                    preds = torch.where(outputs > 0.5, torch.ones([1], dtype=torch.long).cuda(), torch.zeros([1], dtype=torch.long).cuda())
                    iouEvalVal.addBatch(preds.squeeze(), targets[:, 0]) # no_grad handles it already
                    #print ("Time to add confusion matrix: ", time.time() - start_time_iou)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs-save', type=int, default=20)    #You can use this value to save model every X epochs
